"use client";

import { useAuth } from "@/context/AuthContext";
import { useState, useEffect, useMemo } from "react";
import {
    Search,
    BookOpen,
    Sparkles,
    Lightbulb,
    Bookmark,
    ChevronRight,
    BookMarked,
    FileText,
    Lock,
    Zap
} from "lucide-react";
import { useRouter } from "next/navigation";
import { Badge } from "@/components/ui/Badge";
import BottomNav from "@/components/layout/BottomNav";
import { EmptyState } from "@/components/ui/EmptyState";
import { LibraryService, LibraryItem } from "@/services/student/library.service";
import { SubscriptionPlan } from "@/types/finance";
import { Card } from "@/components/ui/Card";
import { Button } from "@/components/ui/Button";

export default function LibraryPage() {
    const { user, subscription, isSuperAdmin } = useAuth();
    const router = useRouter();
    const [activeFilter, setActiveFilter] = useState("Todo");
    const [recommended, setRecommended] = useState<LibraryItem[]>([]);
    const [saved, setSaved] = useState<LibraryItem[]>([]);
    const [loading, setLoading] = useState(true);
    const [showPremiumModal, setShowPremiumModal] = useState(false);

    const isPro = useMemo(() => {
        return subscription?.plan !== SubscriptionPlan.FREE || isSuperAdmin;
    }, [subscription?.plan, isSuperAdmin]);

    useEffect(() => {
        const loadResources = async () => {
            setLoading(true);
            try {
                const [recData, savedData] = await Promise.all([
                    LibraryService.getRecommendedResources(),
                    user ? LibraryService.getSavedResources(user.uid) : Promise.resolve([])
                ]);
                setRecommended(recData);
                setSaved(savedData);
            } catch (error) {
                console.error("Error loading library resources:", error);
            } finally {
                setLoading(false);
            }
        };
        loadResources();
    }, [user]);

    const handleAccessItem = (item: LibraryItem) => {
        // Assume all 'priority' or specific items are PRO for demonstration
        const itemIsPro = item.priority === "PREMIUM" || item.priority === "ESTRATÉGICO";
        
        if (itemIsPro && !isPro) {
            setShowPremiumModal(true);
            return;
        }
        
        // Logical access here
        if (item.id) {
            router.push(`/learning?id=${item.id}`);
        }
    };

    return (
        <div className="min-h-screen bg-[var(--theme-bg-base)] pb-32 font-sans transition-colors duration-500" suppressHydrationWarning>
            {/* Header */}
            <div className="px-6 pt-12 pb-6 sticky top-0 bg-[var(--theme-bg-base)]/80 backdrop-blur-2xl z-40 border-b border-[var(--theme-border-soft)]">
                <div className="flex justify-between items-center mb-4">
                    <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-xl bg-brand-primary/5 flex items-center justify-center text-brand-primary border border-brand-primary/10 shadow-sm">
                            <Sparkles size={18} />
                        </div>
                        <h1 className="text-lg font-bold tracking-tight text-slate-800">Tu Biblioteca</h1>
                    </div>
                    <div className="p-2 rounded-full hover:bg-[var(--theme-bg-surface)] transition-colors cursor-pointer text-[var(--theme-text-tertiary)]">
                        <Search size={20} />
                    </div>
                </div>

                {/* Filters */}
                <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar">
                    {["Todo", "Lectura", "Práctica"].map(filter => (
                        <button
                            key={filter}
                            onClick={() => setActiveFilter(filter)}
                            className={`px-5 py-2 rounded-full text-[10px] font-bold uppercase tracking-wider whitespace-nowrap flex items-center gap-2 transition-all duration-300 ${activeFilter === filter
                                ? "bg-brand-primary text-white shadow-lg shadow-brand-primary/20 scale-105"
                                : "bg-[var(--theme-bg-surface)] border border-[var(--theme-border-soft)] text-[var(--theme-text-tertiary)] hover:border-brand-primary/30"
                                }`}
                        >
                            {filter === "Lectura" && <BookOpen size={10} />}
                            {filter}
                        </button>
                    ))}
                </div>
            </div>

            <div className="px-6 space-y-10 mt-8">
                {/* Recommended Section */}
                <div className="overflow-hidden">
                    <div className="flex items-center gap-2 mb-6">
                        <h2 className="text-base font-bold text-[var(--theme-text-primary)] uppercase tracking-tight italic">Recomendaciones</h2>
                        {recommended.length > 0 && (
                            <div className="w-1.5 h-1.5 rounded-full bg-brand-success animate-pulse" />
                        )}
                    </div>

                    {loading ? (
                        <div className="flex gap-5 overflow-x-auto pb-6 no-scrollbar -mx-6 px-6">
                            {[1, 2].map(i => (
                                <div key={i} className="min-w-[280px] w-[280px] h-[260px] bg-[var(--theme-bg-surface)] rounded-[1.5rem] border border-[var(--theme-border-soft)] animate-pulse shrink-0" />
                            ))}
                        </div>
                    ) : recommended.length === 0 ? (
                        <div
                            onClick={() => router.push('/simulation')}
                            className="py-10 group cursor-pointer transition-all hover:scale-[1.01]"
                        >
                            <EmptyState
                                title="Sin recomendaciones aún"
                                description="Completa tu diagnóstico inicial para que el sistema personalice recursos para ti."
                                icon={BookMarked}
                                className="group-hover:border-brand-primary/40 group-hover:bg-brand-primary/5 transition-colors"
                            />
                        </div>
                    ) : (
                        <div className="flex gap-5 overflow-x-auto pb-6 no-scrollbar -mx-6 px-6">
                            {recommended.map(item => {
                                const itemIsPro = item.priority === "PREMIUM" || item.priority === "ESTRATÉGICO";
                                return (
                                    <div 
                                        key={item.id} 
                                        onClick={() => handleAccessItem(item)}
                                        className="min-w-[280px] w-[280px] bg-[var(--theme-bg-surface)] rounded-[1.5rem] border border-[var(--theme-border-soft)] shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-500 flex flex-col shrink-0 overflow-hidden group cursor-pointer relative"
                                    >
                                        <div className={`aspect-video ${item.image || "bg-brand-primary/10"} relative p-4 overflow-hidden`}>
                                            <Badge className="bg-brand-primary text-white border-0 text-[10px] uppercase font-black tracking-widest absolute top-3 left-3 shadow-lg">
                                                {item.priority || item.type}
                                            </Badge>
                                            {itemIsPro && !isPro && (
                                                <div className="absolute inset-0 bg-brand-primary/20 backdrop-blur-[2px] flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                                                    <div className="bg-white/90 p-3 rounded-2xl shadow-2xl">
                                                        <Lock className="text-brand-primary w-6 h-6" />
                                                    </div>
                                                </div>
                                            )}
                                        </div>
                                        <div className="p-5 flex-1 flex flex-col">
                                            <div className="flex justify-between items-start mb-2">
                                                <h3 className="font-bold text-[var(--theme-text-primary)] text-base leading-tight group-hover:text-brand-primary transition-colors line-clamp-2">{item.title}</h3>
                                                {itemIsPro && !isPro && <Lock className="w-3 h-3 text-[var(--theme-text-tertiary)] mt-1" />}
                                            </div>
                                            <p className="text-[11px] text-[var(--theme-text-tertiary)] mb-5 line-clamp-2 leading-relaxed">{item.desc}</p>
                                            <div className="mt-auto flex justify-between items-center pt-3 border-t border-[var(--theme-border-soft)]">
                                                <Badge variant="ghost" className="bg-[var(--theme-border-soft)]/30 text-[var(--theme-text-tertiary)] text-[9px] font-black px-2 py-1 rounded-md">
                                                    {item.match || item.type}
                                                </Badge>
                                                <button className="text-brand-primary text-[10px] font-black uppercase tracking-widest flex items-center gap-1 group/btn">
                                                    {itemIsPro && !isPro ? 'Desbloquear' : 'Ingresar'} <ChevronRight size={14} className="group-hover/btn:translate-x-1 transition-transform" />
                                                </button>
                                            </div>
                                        </div>
                                    </div>
                                );
                            })}
                        </div>
                    )}
                </div>

                {/* Saved Section */}
                <div className="pb-10">
                    <div className="flex justify-between items-baseline mb-6 px-2">
                        <h2 className="text-base font-bold text-[var(--theme-text-primary)] uppercase tracking-tight italic">Guardados</h2>
                        {!loading && (
                            <span className="text-[10px] font-bold text-[var(--theme-text-tertiary)] uppercase tracking-widest">{saved.length} Elementos</span>
                        )}
                    </div>

                    {loading ? (
                        <div className="space-y-4">
                            {[1, 2].map(i => (
                                <div key={i} className="h-20 bg-[var(--theme-bg-surface)] rounded-2xl animate-pulse" />
                            ))}
                        </div>
                    ) : saved.length === 0 ? (
                        <div className="py-8">
                            <EmptyState
                                title="Nada guardado aún"
                                description="Guarda lecturas y materiales de práctica para acceder a ellos rápidamente."
                                icon={FileText}
                            />
                        </div>
                    ) : (
                        <div className="space-y-4">
                            {saved.map(item => (
                                <div key={item.id} className="bg-[var(--theme-bg-surface)] rounded-2xl p-5 border border-[var(--theme-border-soft)] shadow-sm hover:shadow-md hover:border-brand-primary/20 transition-all flex gap-5 items-center group cursor-pointer" onClick={() => handleAccessItem(item)}>
                                    <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-[var(--theme-bg-base)] to-[var(--theme-border-soft)] shrink-0 group-hover:scale-105 transition-transform flex items-center justify-center text-[var(--theme-text-tertiary)]">
                                        {(item.priority === "PREMIUM" || item.priority === "ESTRATÉGICO") && !isPro ? <Lock size={20} /> : <FileText size={20} />}
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <div className="flex justify-between items-start mb-2">
                                            <h4 className="font-bold text-[var(--theme-text-primary)] text-sm truncate pr-2 group-hover:text-brand-primary transition-colors">{item.title}</h4>
                                            <Bookmark size={16} className="text-brand-primary/30 fill-brand-primary/10 group-hover:text-brand-primary transition-colors shrink-0" />
                                        </div>
                                        <div className="flex items-center gap-3">
                                            <span className="text-[9px] font-black text-[var(--theme-text-tertiary)] uppercase tracking-widest">{item.type}</span>
                                            <span className="w-1 h-1 rounded-full bg-[var(--theme-border-soft)]" />
                                            <span className={`text-[9px] font-black uppercase tracking-widest ${item.status?.toLowerCase() === "continuar" ? "text-brand-primary" : "text-brand-success"}`}>
                                                {item.status?.toLowerCase() === "completado" || item.status?.toLowerCase() === "leído" ? "✓ Completado" : "En curso"}
                                            </span>
                                        </div>
                                    </div>
                                </div>
                            ))}
                        </div>
                    )}
                </div>

                {/* Info Card */}
                <div className="bg-[var(--theme-bg-surface)] rounded-[2.5rem] p-10 border border-[var(--theme-border-soft)] border-dashed text-center relative overflow-hidden mb-12">
                    <div className="absolute top-0 inset-x-0 h-1 bg-gradient-to-r from-transparent via-brand-primary/20 to-transparent" />
                    <Lightbulb size={36} className="text-brand-primary/20 mx-auto mb-5 fill-brand-primary/5" />
                    <h3 className="font-bold text-[var(--theme-text-primary)] mb-3 text-base">Ruta de Aprendizaje</h3>
                    <p className="text-[11px] text-[var(--theme-text-tertiary)] leading-relaxed max-w-[260px] mx-auto font-medium">
                        Guarda recursos estratégicos para optimizar tus sesiones de entrenamiento.
                    </p>
                </div>
            </div>

            {/* Premium Modal (Growth Guardian) */}
            {showPremiumModal && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-[var(--theme-bg-base)]/80 backdrop-blur-xl animate-in fade-in duration-300 shadow-2xl">
                    <Card variant="primary" className="max-w-md w-full p-10 text-center space-y-6 shadow-4k animate-in zoom-in-95 duration-300 relative border-brand-primary/20">
                        <div className="w-20 h-20 rounded-[2rem] bg-brand-primary text-white flex items-center justify-center mx-auto shadow-[0_0_30px_rgba(var(--brand-primary-rgb),0.3)] animate-pulse">
                            <Sparkles size={36} fill="currentColor" />
                        </div>
                        <div className="space-y-2">
                            <h3 className="text-3xl font-black text-[var(--theme-text-primary)] uppercase italic tracking-tightest leading-none">Recurso <span className="text-brand-primary">Elite</span></h3>
                            <p className="text-[11px] text-[var(--theme-text-secondary)] font-bold uppercase tracking-[0.2em] opacity-60">Biblioteca de Nivel Superior</p>
                            <p className="text-sm text-[var(--theme-text-primary)] font-medium leading-relaxed mt-4">
                                Desbloquea guías estratégicas y material exclusivo diseñado por expertos para usuarios <span className="text-brand-primary font-bold">PRO</span>. No te conformes con lo básico, evoluciona tu aprendizaje.
                            </p>
                        </div>
                        <div className="space-y-4 pt-6">
                            <Button 
                                variant="premium" 
                                className="w-full h-16 rounded-2xl shadow-xl hover:scale-[1.02] flex items-center justify-center gap-3 font-black text-xs uppercase tracking-[0.2em] transform transition-all shimmer-gold"
                                onClick={() => router.push('/pricing')}
                            >
                                <Zap className="w-4 h-4 fill-white" /> Adquirir Versión Completa
                            </Button>
                            <Button 
                                variant="ghost" 
                                className="w-full h-12 text-[10px] uppercase font-black tracking-widest text-[var(--theme-text-tertiary)] hover:text-brand-primary transition-colors" 
                                onClick={() => setShowPremiumModal(false)}
                            >
                                Seguir con la versión limitada
                            </Button>
                        </div>
                    </Card>
                </div>
            )}

            <BottomNav />
        </div>
    );
}
