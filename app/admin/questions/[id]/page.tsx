            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div className="flex items-center gap-4">
                    <Link href="/admin/questions">
                        <Button variant="ghost" size="sm" icon={ArrowLeft} className="rounded-full w-12 h-12 p-0 border border-[var(--theme-border-soft)] hover:border-brand-primary/30" />
                    </Link>
                    <div>
                        <h1 className="text-4xl font-black text-[var(--theme-text-primary)] flex items-center gap-4 tracking-tighter italic uppercase">
                            <Database className="text-brand-primary" size={36} /> Editor de Reactivo
                        </h1>
                        <p className="text-[var(--theme-text-tertiary)] text-sm mt-1 flex items-center gap-2 font-medium">
                            <Activity size={14} className="text-brand-primary" /> Transmisión Segura / UUID: <span className="font-mono text-[10px] bg-[var(--theme-bg-surface)] px-2 py-0.5 rounded text-brand-primary border border-[var(--theme-border-soft)]">{id}</span>
                        </p>
                    </div>
                </div>
                <div className="flex gap-3">
                    <Button
                        ...
                        disabled={saving}
                        variant="primary"
                        className="shadow-lg shadow-brand-primary/10 px-8 font-bold uppercase tracking-wider text-[10px]"
                ...
                <div className="lg:col-span-2 space-y-8">
                    <Card variant="solid" className="p-8 space-y-6">
                        ...
                        <div className="space-y-4">
                            <label className="text-[10px] font-black text-[var(--theme-text-secondary)] uppercase tracking-widest ml-1 opacity-60">Enunciado Principal</label>
                            <textarea
                                required
                                rows={6}
                                value={formData.text}
                                onChange={(e) => setFormData({ ...formData, text: e.target.value })}
                                className="w-full bg-[var(--theme-bg-surface)] border border-[var(--theme-border-soft)] rounded-2xl p-6 text-[var(--theme-text-primary)] text-lg font-medium focus:ring-1 focus:ring-brand-primary/50 focus:border-brand-primary/50 outline-none transition-all resize-none placeholder:text-[var(--theme-text-tertiary)] shadow-inner"
                            />
                        </div>

                        <div className="space-y-4">
                            <label className="text-[10px] font-black text-[var(--theme-text-secondary)] uppercase tracking-widest ml-1 opacity-60">Imagen de Apoyo (URL)</label>
                            <Input
                                value={formData.imageUrl}
                                onChange={(e) => setFormData({ ...formData, imageUrl: e.target.value })}
                                icon={ImageIcon}
                                className="bg-[var(--theme-bg-surface)] border-[var(--theme-border-soft)] shadow-inner"
                                placeholder="https://servidor.com/imagen.jpg"
                            />
                            {formData.imageUrl && (
                                <div className="relative rounded-2xl overflow-hidden border border-[var(--theme-border-soft)] mt-2 group aspect-video bg-[var(--theme-bg-surface)]">
                    ...
                    <Card variant="solid" className="p-8 space-y-8">
                        <div className="flex items-center gap-2 text-[var(--theme-text-primary)] border-b border-[var(--theme-border-soft)] pb-4">
                        ...
                        <div className="grid gap-4">
                            {["A", "B", "C", "D"].map((opt) => (
                                <div key={opt} className={`flex gap-4 items-center p-3 rounded-2xl bg-[var(--theme-bg-surface)] border transition-all ${formData.correctAnswer === opt ? "border-green-500/30 bg-green-500/[0.02]" : "border-[var(--theme-border-soft)]"}`}>
                                    <button
                                        type="button"
                                        onClick={() => setFormData({ ...formData, correctAnswer: opt })}
                                        className={`w-12 h-12 flex items-center justify-center rounded-xl font-black transition-all ${formData.correctAnswer === opt ? "bg-green-500 text-white shadow-[0_0_20px_rgba(34,197,94,0.3)]" : "bg-[var(--theme-bg-base)] text-[var(--theme-text-secondary)] hover:text-[var(--theme-text-primary)]"}`}
                                    >
                                        {opt}
                                    </button>
                                    <input
                                        ...
                                        className="flex-1 bg-transparent border-none focus:ring-0 text-[var(--theme-text-primary)] py-3 placeholder:text-[var(--theme-text-tertiary)]"
                                    />
                    ...
                    <Card variant="solid" className="p-8 space-y-8">
                        ...
                        <div className="space-y-6">
                            <div className="space-y-2">
                                <label className="text-[10px] font-black text-[var(--theme-text-secondary)] uppercase tracking-widest ml-1 opacity-60">Módulo</label>
                                <Select
                                    required
                            ...
                            <div className="space-y-2">
                                <label className="text-[10px] font-black text-[var(--theme-text-secondary)] uppercase tracking-widest ml-1 opacity-60">Nivel</label>
                                <Select
                                    required
                        ...
                        <div className="space-y-4 pt-4 border-t border-[var(--theme-border-soft)]">
                            <label className="text-[10px] font-black text-[var(--theme-text-secondary)] uppercase tracking-widest ml-1 opacity-60">Explicación Académica</label>
                            <textarea
                                rows={4}
                                value={formData.explanation}
                                onChange={(e) => setFormData({ ...formData, explanation: e.target.value })}
                                className="w-full bg-[var(--theme-bg-surface)] border border-[var(--theme-border-soft)] rounded-2xl p-4 text-xs italic text-[var(--theme-text-secondary)]"
                            />
                        </div>
                        ...
                        <Button
                            type="submit"
                            variant="primary"
                            className="w-full h-14 font-black uppercase tracking-widest shadow-lg shadow-brand-primary/10"
                            icon={Save}
                            disabled={saving}
                        >
                            Guardar Cambios
