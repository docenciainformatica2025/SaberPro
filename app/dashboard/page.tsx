                ...
                {loading || isLoadingData ? (
                    ...
                    <div className="space-y-12 pb-44 animate-in fade-in duration-700">
                        ...
                        <div id="tour-welcome" className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                            <div>
                                <h1 className="text-4xl font-black bg-clip-text text-transparent bg-gradient-to-r from-white via-metal-silver to-white flex items-center gap-4 tracking-tighter italic uppercase">
                                    <Sparkles className="text-metal-gold" size={36} /> Hola, {userName?.split(' ')[0]}
                                </h1>
                                <p className="text-metal-silver/60 text-sm mt-2 flex items-center gap-2 font-medium italic">
                        ...
                        >
                            <motion.div variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0 } }}>
                                <StatCardPremium title="Puntaje Global" value={stats.averageScore} icon={<Target size={22} />} trend={stats.averageScore > 300 ? "Nivel Avanzado" : "En Progreso"} trendUp={stats.averageScore > 300} color="gold" />
                            </motion.div>
                            <motion.div variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0 } }}>
                                <StatCardPremium title="Simulacros Ejecutados" value={stats.completedSimulations} icon={<Brain size={22} />} trend="+2 esta semana" trendUp={true} color="purple" />
                            </motion.div>
                            <motion.div variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0 } }}>
                                <StatCardPremium title="Nivel de Preparación" value={`${Math.min(100, Math.round((stats.completedSimulations * 10) + (stats.averageScore / 5)))}%`} icon={<Zap size={22} />} trend="Listo para el examen" trendUp={true} color="green" />
                            </motion.div>
                        </motion.div>
                            ...
                            <div className="lg:col-span-2 space-y-8">
                                ...
                                <Card variant="solid" className="p-8 border-white/5">
                                    ...
                                    {recentResults.length > 0 ? (
                                        ...
                                        <div className="py-6">
                                            <EmptyState title="Sin Actividad Reciente" description="Aún no has realizado simulacros. Tu progreso aparecerá aquí una vez que comiences." icon={History} actionLabel="Realizar Primer Test" onAction={() => router.push('/simulation')} />
                                        </div>
                                    )}
                                </Card>
                            </div>
                                ...
                                <Card id="tour-join-class" variant="solid" className="p-8 border-white/5 bg-gradient-to-b from-white/[0.03] to-transparent">
                                    ...
                                    <div className="space-y-4">
                                        <div className="relative group">
                                            <Input placeholder="CÓDIGO DE ACCESO" value={joinCode} onChange={(e) => setJoinCode(e.target.value)} className="bg-black/40 border-white/10 text-center font-mono tracking-[0.2em] text-lg uppercase placeholder:text-metal-silver/20 focus:border-purple-500/50 transition-all rounded-xl py-6" maxLength={6} />
                                        ...
                                        <Button variant="premium" className="w-full font-black uppercase tracking-widest text-xs h-12 shadow-[0_0_20px_rgba(168,85,247,0.2)] hover:shadow-[0_0_30px_rgba(168,85,247,0.4)] bg-gradient-to-r from-purple-600 to-indigo-600 border-none" onClick={handleJoinClass} isLoading={joining} disabled={!joinCode}>
                                            Validar Acceso
                                        </Button>
                                    </div>
                                    {myClasses.length > 0 && (
                                ...
                                {(!subscription || subscription.plan === 'free') && (
                                    <Card variant="premium" className="p-8 relative overflow-hidden text-center border-metal-gold/30">
                                        ...
                                        <div className="relative z-10">
                                            ...
                                            <p className="text-xs text-metal-silver/80 mb-6 font-medium leading-relaxed">Habilita todas las herramientas de análisis y práctica.</p>
                                            <Link href="/pricing">
                                                <Button variant="premium" className="w-full font-black uppercase tracking-widest text-xs h-10 shadow-[0_0_20px_rgba(212,175,55,0.3)]">Obtener Pro</Button>
                                            </Link>
                                        </div>
                                    </Card>
                                )}
