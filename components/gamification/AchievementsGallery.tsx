...
const ALL_BADGES = [
    ...
    { id: 'perfect_score', name: 'Puntaje Perfecto', icon: Target, color: 'text-brand-primary', desc: 'Responde todas las preguntas correctamente.' },
    ...
    { id: 'streak_7', name: 'Constancia Oro', icon: Trophy, color: 'text-brand-primary', desc: 'Mantén una racha de 7 días.' },
];
    ...
    return (
        <section className="space-y-4">
            <div className="flex items-center justify-between">
                <h3 className="text-xs font-semibold uppercase tracking-[0.2em] text-theme-text-secondary/40">Insignias y Logros</h3>
                ...
                {ALL_BADGES.map((badge, idx) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            `}>
                                <Icon size={20} className={isUnlocked ? badge.color : 'text-theme-text-secondary'} />
                            </div>

                            {/* Hover Tooltip */}
                            <div className="absolute -top-12 left-1/2 -translate-x-1/2 w-32 bg-slate-900 border border-white/10 p-2 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-20 text-center shadow-2xl">
                                <p className="text-[10px] font-semibold text-white uppercase mb-0.5">{badge.name}</p>
                                <p className="text-[9px] text-theme-text-secondary leading-tight">{badge.desc}</p>
