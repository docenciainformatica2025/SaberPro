...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
    ({ className, variant = 'primary', size = 'md', isLoading, icon: Icon, iconPosition = 'left', children, ...props }, ref) => {
        ...
        const baseStyles = cn(
            "inline-flex items-center justify-center rounded-[var(--btn-radius)] font-bold",
        ...
        const variants = {
            primary: "bg-brand-primary text-[var(--brand-primary-fg)] shadow-md hover:shadow-lg active:scale-[0.98]",
            success: "bg-brand-success text-white shadow-md hover:brightness-110 active:scale-[0.98]",
            accent: "bg-brand-accent text-white shadow-md hover:brightness-110 active:scale-[0.98]",
            error: "bg-brand-error text-white shadow-md hover:brightness-110 active:scale-[0.98]",
            ...
            premium: "bg-gradient-to-br from-brand-primary via-brand-primary to-black/80 text-white shadow-xl hover:brightness-110 border border-white/20",
            maestro: "btn-maestro text-[var(--brand-primary-fg)] font-black uppercase tracking-[0.2em] shadow-premium",
        };

        const sizes = {
            sm: "px-3 h-[var(--btn-h-sm)] text-xs",
            md: "px-5 h-[var(--btn-h-md)] text-sm",
            lg: "px-7 h-[var(--btn-h-lg)] text-base",
            xl: "px-9 h-[var(--btn-h-xl)] text-lg",
            icon: "h-[var(--btn-h-md)] w-[var(--btn-h-md)]"
        };
